...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn attest(origin, identity_hash: T::Hash, attestation: Attestation) -> DispatchResult {
			let _sender = ensure_signed(origin)?;
		...
		pub fn register_and_attest(origin, identity_type: IdentityType, identity: Identity, attestation: Attestation) -> DispatchResult {
			let _sender = ensure_signed(origin)?;
			// create hash
			let mut buf = Vec::new();
			buf.extend_from_slice(&identity_type.encode());
		...
		pub fn verify(origin, identity_hash: T::Hash, verifier_index: u32) -> DispatchResult {
			let _sender = ensure_signed(origin)?;
		...
		/// The verification is handled by a set of seeded verifiers who run
		/// the off-chain worker node to verify attestations.
		pub fn deny(origin, identity_hash: T::Hash, verifier_index: u32) -> DispatchResult {
			let _sender = ensure_signed(origin)?;
			ensure!((verifier_index as usize) < Self::verifiers().len(), "Verifier index out of bounds");
			ensure!(Self::verifiers()[verifier_index as usize] == _sender.clone(), "Sender is not a verifier");
		...
		pub fn verify_many(origin, identity_hashes: Vec<T::Hash>, verifier_index: u32) -> DispatchResult {
			let _sender = ensure_signed(origin)?;
			ensure!((verifier_index as usize) < Self::verifiers().len(), "Verifier index out of bounds");
			ensure!(Self::verifiers()[verifier_index as usize] == _sender.clone(), "Sender is not a verifier");
		...
		pub fn deny_many(origin, identity_hashes: Vec<T::Hash>, verifier_index: u32) -> DispatchResult {
			let _sender = ensure_signed(origin)?;
			ensure!((verifier_index as usize) < Self::verifiers().len(), "Verifier index out of bounds");
			ensure!(Self::verifiers()[verifier_index as usize] == _sender.clone(), "Sender is not a verifier");
		...
		pub fn add_metadata(origin, identity_hash: T::Hash, avatar: Vec<u8>, display_name: Vec<u8>, tagline: Vec<u8>) -> DispatchResult {
			let _sender = ensure_signed(origin)?;
			...
			ensure!(<frame_system::Module<T>>::block_number() <= record.expiration_length, "Identity expired");
		...
		pub fn revoke(origin, identity_hash: T::Hash) -> DispatchResult {
			let _sender = ensure_signed(origin)?;
			let record = <IdentityOf<T>>::get(&identity_hash).ok_or("Identity does not exist")?;
			// Check that original sender and current sender match
			ensure!(record.account == _sender, "Stored identity does not match sender");
...
impl<T: Trait> Module<T> {
	...
	fn verify_or_deny_identity(sender: T::AccountId, identity_hash: &T::Hash, approve: bool) -> DispatchResult {
		let record = <IdentityOf<T>>::get(identity_hash).ok_or("Identity does not exist")?;
		ensure!(<frame_system::Module<T>>::block_number() <= record.expiration_length, "Identity expired");
		match record.stage {
			IdentityStage::Registered => return Err(Error::<T>::NoAttestationToVerify)?,
			IdentityStage::Verified => return Err(Error::<T>::AlreadyVerified)?,
	...
	fn do_attest(sender: T::AccountId, identity_hash: T::Hash, attestation: Attestation) -> DispatchResult {
		let record = <IdentityOf<T>>::get(&identity_hash).ok_or("Identity does not exist")?;
...
decl_storage! {
	trait Store for Module<T: Trait> as Identity {
		...
		pub IdentityOf get(fn identity_of): map hasher(blake2_256) T::Hash => Option<IdentityRecord<T::AccountId, T::BlockNumber>>;
		...
		pub UsedTypes get(fn used_types): map hasher(blake2_256) T::AccountId => Vec<IdentityType>;
		/// Verifier set
