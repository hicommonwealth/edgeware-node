...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn force_create(origin,
			...
			<Self as ManageableAsset<_>>::force_create_asset(id, owner, max_zombies, min_balance)?;
		}
		...
		fn force_destroy(origin,
			...
			<Self as ManageableAsset<_>>::force_destroy_asset(id, zombies_witness)
		}
...
impl<T: Config> ManageableAsset<T::AccountId> for Module<T> {
	...
	fn force_create_asset(id: Self::AssetId, owner: T::AccountId, max_zombies: u32, min_balance: Self::Balance)
		-> DispatchResult
	{
		ensure!(!Asset::<T>::contains_key(id), Error::<T>::InUse);
		ensure!(!min_balance.is_zero(), Error::<T>::MinBalanceZero);
