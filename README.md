...
Users are able to run `edgeware-node` and included recommended chainspec at launch, as is. For the Edgeware Mainnet Launch, we recommend the chainspec with the following hashes. It can be generated by running `shasum` and `b2sum` on the file `mainnet/chainspec.json`. 
```
