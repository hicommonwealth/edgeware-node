...
For the Edgeware Mainnet Launch, we recommend the chainspec with the following hashes. The hashes can be generated by running `shasum` and `b2sum` on the file `mainnet/chainspec.json`. 
```
➜  edgeware-node git:(develop) ✗ shasum mainnet/chainspec.json
64a453e34ae60439fd1610b33a22d4e4bbf9c762  mainnet/chainspec.json
➜  edgeware-node git:(develop) ✗ b2sum mainnet/chainspec.json
5565f32bd8e9059acc6d707e6551d4f767d95a5a061865320351b59bceccbfa1e529692bc03eb2ae1891997be440348172cddcae846996e14a416ea2342bae7a  mainnet/chainspec.json
```

At launch, you can run `edgeware-node` and recommended chainspec, as is. However, individuals are able to independently regenerate and verify that the recommended `mainnet/chainspec.json` hashes follow the `edgeware-lockdrop` specification. To do this, you'll need to:
