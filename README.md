...
For the Edgeware Mainnet Launch, we recommend the chainspec with the following hashes. Users are able to run `edgeware-node` and included recommended chainspec at launch, as is. It can be generated by running `shasum` and `b2sum` on the file `mainnet/chainspec.json`. 
```
...
Validators are able to independently regenerate and verify that the `mainnet/chainspec.json` hashes are equivalent to the `edgeware-lockdrop` specification. To do this, you'll need to:
