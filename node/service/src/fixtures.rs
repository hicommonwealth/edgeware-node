use edgeware_primitives::BlockNumber;
...
pub fn get_spec_allocation() 
	-> Result<(
		...
		Vec<(AccountId, AccountId, AuraId, Balance)>
	)> {
	...
	let validators: Vec<(AccountId, AccountId, AuraId, Balance)> = validators_json.into_iter().map(|e| {
		return (
			<[u8; 32]>::from_hex(e.0).unwrap().unchecked_into(),
...
pub fn get_commonwealth_validators() -> Vec<(AccountId, AccountId, AuraId, Balance)> {
	return vec![(
