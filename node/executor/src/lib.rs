...
mod tests {
	...
	use edgeware_runtime::impls::LinearWeightToFee;
	use edgeware_primitives::{Balance, Hash, BlockNumber};
	...
	const BLOATY_CODE: &[u8] = edgeware_runtime::WASM_BINARY_BLOATY;
	...
	fn new_test_ext(code: &[u8], support_changes_trie: bool) -> TestExternalities<Blake2Hasher> {
		let mut ext = TestExternalities::new_with_code(
			code,
			edgeware_testing::genesis::config(support_changes_trie, Some(code)).build_storage().unwrap(),
		);
...
"#;
	...
	fn should_import_block_with_test_client() {
		use edgeware_testing::client::{ClientExt, TestClientBuilderExt, TestClientBuilder, consensus::BlockOrigin};

		let client = TestClientBuilder::new().build();
		let block1 = changes_trie_block();
		let block_data = block1.0;
		let block = edgeware_primitives::Block::decode(&mut &block_data[..]).unwrap();
