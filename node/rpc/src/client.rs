...
impl sc_client_api::UsageProvider<Block> for Client {
	fn usage_info(&self) -> sc_client_api::ClientInfo<Block> {
		match self {
			Self::Edgeware(client) => client.usage_info(),
		}
	}
}
...
impl sc_client_api::BlockBackend<Block> for Client {
	...
	fn block(&self, id: &BlockId<Block>) -> sp_blockchain::Result<Option<SignedBlock<Block>>> {
		match self {
			Self::Edgeware(client) => client.block(id),
		}
	}

	fn block_status(&self, id: &BlockId<Block>) -> sp_blockchain::Result<BlockStatus> {
		match self {
			Self::Edgeware(client) => client.block_status(id),
		}
	}

	fn justifications(&self, id: &BlockId<Block>) -> sp_blockchain::Result<Option<Justifications>> {
		match self {
			Self::Edgeware(client) => client.justifications(id),
		}
	}
...
impl sc_client_api::StorageProvider<Block, crate::FullBackend> for Client {
	...
	fn storage_hash(
		...
		match self {
			Self::Edgeware(client) => client.storage_hash(id, key),
		}
	}
	...
	fn storage_pairs(
		...
		match self {
			Self::Edgeware(client) => client.storage_pairs(id, key_prefix),
		}
	}
	...
	fn child_storage(
		...
		match self {
			Self::Edgeware(client) => client.child_storage(id, child_info, key),
		}
	}
	...
	fn child_storage_keys(
		...
		match self {
			Self::Edgeware(client) => client.child_storage_keys(id, child_info, key_prefix),
		}
	}
	...
	fn child_storage_hash(
		...
		match self {
			Self::Edgeware(client) => client.child_storage_hash(id, child_info, key),
		}
	}
	...
	fn max_key_changes_range(
		...
		match self {
			Self::Edgeware(client) => client.max_key_changes_range(first, last),
		}
	}
	...
	fn key_changes(
		...
		match self {
			Self::Edgeware(client) => client.key_changes(first, last, storage_key, key),
		}
	}
}
...
impl sp_blockchain::HeaderBackend<Block> for Client {
	fn header(&self, id: BlockId<Block>) -> sp_blockchain::Result<Option<Header>> {
		match self {
			Self::Edgeware(client) => client.header(&id),
		}
	}

	fn info(&self) -> sp_blockchain::Info<Block> {
		match self {
			Self::Edgeware(client) => client.info(),
		}
	}

	fn status(&self, id: BlockId<Block>) -> sp_blockchain::Result<sp_blockchain::BlockStatus> {
		match self {
			Self::Edgeware(client) => client.status(id),
		}
	}

	fn number(&self, hash: Hash) -> sp_blockchain::Result<Option<BlockNumber>> {
		match self {
			Self::Edgeware(client) => client.number(hash),
		}
	}

	fn hash(&self, number: BlockNumber) -> sp_blockchain::Result<Option<Hash>> {
		match self {
			Self::Edgeware(client) => client.hash(number),
		}
	}
}
