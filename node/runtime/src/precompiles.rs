	...
	<R::Call as Dispatchable>::Origin: From<Option<R::AccountId>>,
{
	fn execute(
		...
		match address {
			...
			a if a == hash(1026) => Some(Sha3FIPS512::execute(input, target_gas, context)),
			a if a == hash(1027) => Some(ECRecoverPublicKey::execute(input, target_gas, context)),
			a if a == hash(1028) => Some(Ed25519Verify::execute(input, target_gas, context)),
			_ => None,
		}
	}
}
