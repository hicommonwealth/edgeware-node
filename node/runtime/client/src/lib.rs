...
pub use edgeware_runtime as runtime;
pub use edgeware_cli::chain_spec::development_config_genesis;
use sp_core::{ChangesTrieConfiguration, map, twox_128};
use sp_core::storage::{ChildInfo, Storage, StorageChild};
use edgeware_runtime::GenesisConfig;
...
pub type Backend = substrate_test_client::Backend<edgeware_runtime::Block>;
...
pub type LightBackend = substrate_test_client::LightBackend<edgeware_runtime::Block>;
...
pub type LightExecutor = GenesisCallExecutor<
	...
	LocalCallExecutor<
		light_backend::Backend<
			sc_client_db::light::LightStorage<edgeware_runtime::Block>,
			HashFor<edgeware_runtime::Block>
		>,
		NativeExecutor<LocalExecutor>
	>
>;
...
impl GenesisParameters {
	fn genesis_config(&self) -> GenesisConfig {
		let config = development_config_genesis();
...
fn additional_storage_with_genesis(genesis_block: &edgeware_runtime::Block) -> BTreeMap<Vec<u8>, Vec<u8>> {
	map![
...
pub trait TestClientBuilderExt<B>: Sized {
	...
	fn build_with_longest_chain(self) -> (Client<B>, LongestChain<B, edgeware_runtime::Block>);
...
> {
	...
	fn build_with_longest_chain(self) -> (Client<Backend>, LongestChain<Backend, edgeware_runtime::Block>) {
...
pub struct LightFetcher {
	call: MaybeFetcherCallback<RemoteCallRequest<edgeware_runtime::Header>, Vec<u8>>,
	body: MaybeFetcherCallback<RemoteBodyRequest<edgeware_runtime::Header>, Vec<edgeware_runtime::Extrinsic>>,
}
...
pub fn needed_extrinsics(
	i: u64,
) -> Vec<edgeware_runtime::UncheckedExtrinsic> {
	let timestamp = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)
		.expect("now always later than unix epoch; qed")
		.as_millis() + (i * edgeware_runtime::constants::time::SLOT_DURATION / 2) as u128;

	vec![
		edgeware_runtime::UncheckedExtrinsic {
			function: edgeware_runtime::Call::Timestamp(pallet_timestamp::Call::set(
